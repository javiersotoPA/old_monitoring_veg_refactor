...
data = r"C:\javi\in_progress\veg_data_formating\quadrats\annex2\Vegetation surveys 2021  - Annex 2 - Survey data (A3629439)\Annex 2B - Quadrat data 2015-2021.csv"

df = pd.read_csv(data)
df.columns= df.columns.str.lower()
print(df)

df["quadrat_id"] = df['site'].astype(str) + "__" + df["area"] + "__" + df["quadrat"] + "__" + df["date"]  ## create unique quadrat ID
#print(df)
...
list_qinfo_columns = ['date','site','bog_type','area','quadrat','x','y','quadrat_size_m',
'avg_canopy_height_cm','dung_presence',
'cause_damaged_peat',
'main_calluna_growth_stage','constraints','notes', 'quadrat_id']
...
list_todrop_columns = ['date','site','bog_type','area','quadrat','x','y','quadrat_size_m',
'avg_canopy_height_cm','dung_presence','cause_damaged_peat',
'main_calluna_growth_stage','constraints','notes']
...
execute_values(conn, df_protocol_c, 'test_old_monitoring.quadrat_vegetation')
execute_values(conn, df_qinfo, 'test_old_monitoring.quadrat_info')
